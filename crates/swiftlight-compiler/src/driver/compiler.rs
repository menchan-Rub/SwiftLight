    ...
    pub fn compile<P: AsRef<Path>>(&mut self, source_path: P, output_path: P) -> Result<CompileResult> {
        ...
        let codegen_options = CodegenOptions {
            ...
            inline_threshold: match self.options.optimization_level {
                ...
                OptimizationLevel::O1 => Some(100),
                OptimizationLevel::O2 => Some(250),
                OptimizationLevel::O3 => Some(500),
                OptimizationLevel::Os => Some(75),
                OptimizationLevel::Oz => Some(25),
            },
            unroll_threshold: match self.options.optimization_level {
                OptimizationLevel::O0 => None,
                OptimizationLevel::O1 => Some(16),
                OptimizationLevel::O2 => Some(100),
                OptimizationLevel::O3 => Some(250),
                OptimizationLevel::Os => Some(8),
                OptimizationLevel::Oz => None,
            },
            vectorize: self.options.optimization_level >= OptimizationLevel::O2,
            auto_simd: self.options.optimization_level >= OptimizationLevel::O2,
            profiling: self.options.profiling,
            stack_protector: self.options.stack_protector,
            function_attrs: self.options.function_attrs.clone(),
            optimization_profile: match self.options.size_level {
                0 => OptimizationProfile::Speed,
                1 => OptimizationProfile::Balanced,
                _ => OptimizationProfile::Size,
            },
            lto: self.options.lto,
            parallel_codegen: self.options.parallel_codegen,
            memory_model: match self.options.target_triple.as_deref() {
                Some(triple) if triple.contains("wasm") => MemoryModel::AcquireRelease,
                _ => MemoryModel::SequentiallyConsistent,
            },
            exception_model: match self.options.target_triple.as_deref() {
                Some(triple) if triple.contains("windows") => ExceptionModel::WinEH,
                Some(triple) if triple.contains("wasm") => ExceptionModel::None,
                _ => ExceptionModel::DwarfEH,
            },
            collect_stats: self.options.collect_stats,
            verify_generated_code: self.options.verify_generated_code,
            speculative_optimization: self.options.optimization_level >= OptimizationLevel::O3,
            hot_code_reoptimization: self.options.optimization_level >= OptimizationLevel::O3,
            auto_parallelization: self.options.optimization_level >= OptimizationLevel::O2 && self.options.auto_parallelization,
            cache_aware_optimization: self.options.optimization_level >= OptimizationLevel::O2,
        };
